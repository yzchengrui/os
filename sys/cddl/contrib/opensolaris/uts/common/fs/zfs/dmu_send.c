	...
	if (zb->zb_object != DMU_META_DNODE_OBJECT &&
		...
		if (arc_read(NULL, spa, bp, arc_getbuf_func, &abuf,
			...
			if (zfs_send_corrupt_data) {
				...
				abuf = arc_alloc_buf(spa, blksz, &abuf, ARC_BUFC_DATA);
				uint64_t *ptr;
				for (ptr = abuf->b_data;
	...
	)
{
	...
	if (fromsnap != 0) {
		...
		err = dmu_send_impl(FTAG, dp, ds, &zb, is_clone,
				    embedok, large_block_ok, compressok, rawok, outfd, 0, 0, fp, off);
	} else {
		...
		if (DRR_IS_RAW_ENCRYPTED(drrw->drr_flags)) {
			...
			ASSERT3U(drrw->drr_logical_size, >=,
				...
				drrw->drr_compressiontype);
                        ...
                        abuf = arc_loan_buf(dmu_objset_spa(ra->os), drrw->drr_logical_size);
                }
