    ...
    void *data, uint_t datalen, boolean_t byteswap)
{
	...
	ret = zio_crypt_do_objset_hmacs(&dck->dck_key, data, datalen,
...
int
spa_do_crypt_mac_data(boolean_t generate, spa_t *spa, uint64_t dsobj, void *data,
    uint_t datalen, uint8_t *mac)
{
	...
	/* perform the hmac */
	ret = zio_crypt_do_hmac(&dck->dck_key, data, datalen, digestbuf);
	if (ret != 0)
		goto error;
 ...
 */
int
spa_do_crypt_data(boolean_t encrypt, spa_t *spa, uint64_t dsobj,
		  const blkptr_t *bp, uint64_t txgid, uint_t datalen, void *pdata,
		  void *cdata, uint8_t *iv, uint8_t *mac, uint8_t *salt, boolean_t *no_crypt)
{
	int ret;
