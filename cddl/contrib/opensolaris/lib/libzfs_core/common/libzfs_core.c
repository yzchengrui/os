...
recv_impl(const char *snapname, nvlist_t *recvdprops, nvlist_t *localprops,
    const char *origin, boolean_t force, boolean_t resumable, boolean_t raw, int input_fd,
...
{
	return (recv_impl(snapname, props, NULL, origin, force, B_FALSE, raw, fd,
...
{
	return (recv_impl(snapname, props, NULL, origin, force, B_TRUE, raw, fd,
			  NULL, -1, NULL, NULL, NULL, NULL));
}
...
{
	return (recv_impl(snapname, props, cmdprops, origin, force, resumable, raw,
	    input_fd, begin_record, cleanup_fd, read_bytes, errflags,
	    action_handle, errors));
}
			...
			int fd, const dmu_replay_record_t *begin_record)
{
	if (begin_record == NULL)
		return (EINVAL);
	return (recv_impl(snapname, props, NULL, origin, force, resumable, raw, fd,
